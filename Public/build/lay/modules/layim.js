/** layui-v1.0.5 LGPL license By www.layui.com */ ;
layui.define(["layer", "laytpl", "upload"], function(i) {
	...
	var u = function(i) {
		...
		c = ['<div class="layui-layim-main">', '<div class="layui-layim-info">',
			...
			"</div>", '<ul class="layui-layim-tab{{# if(!d.base.isfriend || !d.base.isgroup){ }}', " layim-tab-two",
			...
			'<ul class="layui-layim-list {{# if(spread === "true"){ }}', " layim-show", '{{# } }}">', u({
				type: "friend",
				item: "item.list",
				index: "index"
			}), "</ul>", "</li>", "{{# }); if(d.friend.length === 0){ }}",
			'<li><ul class="layui-layim-list layim-show"><li class="layim-null">暂无联系人</li></ul>', "{{# } }}", "</ul>",
			'<ul class="layim-tab-content {{# if(!d.base.isfriend && d.base.isgroup){ }}layim-show{{# } }}">', "<li>",
			'<ul class="layui-layim-list layim-show layim-list-group">', u({
				type: "group"
			}), "</ul>", "</li>", "</ul>",
			'<ul class="layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layim-show{{# } }}">', "<li>",
			'<ul class="layui-layim-list layim-show layim-list-history">', u({
				type: "history"
			}), "</ul>", "</li>", "</ul>", '<ul class="layim-tab-content">', "<li>",
			'<ul class="layui-layim-list layim-show" id="layui-layim-search"></ul>', "</li>", "</ul>",
			'<ul class="layui-layim-tool">',
			'<li class="layui-icon layim-tool-search" layim-event="search" title="搜索">&#xe615;</li>',
			'<li class="layui-icon layim-tool-skin" layim-event="skin" title="换肤">&#xe61b;</li>', "{{# if(d.base.find){ }}",
		...
		y = ['<ul class="layui-layim-skin">', "{{# layui.each(d.skin, function(index, item){ }}",
			'<li><img layim-event="setSkin" src="{{ item }}"></li>', "{{# }); }}",
			'<li layim-event="setSkin"><cite>默认</cite></li>', "</ul>"
		].join(""),
		m = ['<div class="layim-chat layim-chat-{{d.data.type}}">', '<div class="layim-chat-title">',
			...
			"</a>", "</div>", '<div class="layim-chat-main">', "<ul></ul>", "</div>", '<div class="layim-chat-footer">',
	...
	layui.data.date = function(i) {
		...
		return a.getFullYear() + "-" + f(a.getMonth() + 1) + "-" + f(a.getDate()) + " " + f(a.getHours()) + ":" + f(a.getMinutes()) +
			":" + f(a.getSeconds())
		...
		return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(
			...
			/\s{2}/g, "&nbsp").replace(/img\[([^\s]+?)\]/g, function(i) {
			...
			return a ? '<a class="layui-layim-file" href="' + a +
				'" target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (e || a) + "</cite></a>" : i
	...
	var p, h, g, v, b, x = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>',
		...
		k =
		'<li class="layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>',
		w = function(i, a, n) {
			return i = i || {}, e.ajax({
				url: i.url,
				type: i.type || "get",
		...
		S = function(i) {
			...
			return C = e.extend(C, l), i.brief ? layui.each(r.ready, function(i, a) {
				...
				C = e.extend(C, s), I(n(c).render(s)), (l.close || i.min) && j(), layui.each(r.ready, function(i, a) {
					a && a(s)
				})
			}, "INIT")
		},
		I = function(i) {
			return t.open({
				...
				success: function(i) {
					...
					}), C.base.right && i.css("margin-left", "-" + C.base.right), h && t.close(h.attr("times"));
					var l = [],
		...
		j = function(i) {
			return h && t.close(h.attr("times")), p && p.hide(), C.mine = C.mine || {}, t.open({
				...
				offset: "rb",
				resize: !1,
				content: '<img src="' + (C.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i ||
					C.base.title || "我的LayIM") + "</span>",
		...
		L = function(i) {
			...
			if (a[0]) {
				...
				return "none" === g.css("display") && g.show(), v && t.close(v.attr("times")), 1 !== s.find("li").length || o[0] ||
					(g.css("width", "800px"), s.css("display", "inline-block")), o[0] || (s.append(n(k).render(l)), a.append(n(m).render(
						l))), z(s.find(".layim-chatlist-" + i.type + i.id)), o[0] || N(), H(i), $(), b
			}
			var d = b = t.open({
				type: 1,
				area: ["600px", "520px"],
				...
				success: function(a) {
					...
					g = a, g.css({
						"background-image": n ? "url(" + n + ")" : "none"
					}), $(), H(i), layui.each(r.chatChange, function(i, a) {
						a && a(q())
					}), N(), E(), a.on("dblclick", ".layui-layim-photos", function() {
						var i = this.src;
		...
		z = function(i, a) {
			...
			var n = i.index() === -1 ? 0 : i.index(),
				...
				o = g.find(l).eq(n);
			if (a) {
				i.hasClass(s) && z(0 === n ? i.next() : i.prev()), i.remove(), o.remove();
				var d = g.find(l).length;
				return 1 === d && (g.find(".layim-chat-list").hide(), g.css("width", "600px")), 0 === d && t.close(b), !1
			}
			i.addClass(s).siblings().removeClass(s), o.css("display", "inline-block").siblings(l).hide(), o.find("textarea").focus(),
		...
		A = function(i) {
			...
			var a = e(".layim-chatlist-" + i.type + i.id),
				t = {},
				l = a.index();
			if (i.timestamp = i.timestamp || (new Date).getTime(), D(i), !g && i.content || l === -1) {
		...
		D = function(i) {
			var a = layui.data("layim")[C.mine.id] || {},
				e = a.chatlog || {};
			e[i.type + i.id] ? (e[i.type + i.id].push(i), e[i.type + i.id].length > o && e[i.type + i.id].shift()) : e[i.type +
				i.id] = [i], a.chatlog = e, layui.data("layim", {
				key: C.mine.id,
				value: a
			})
		},
		...
		R = function(i) {
			var a, e = p.find(".layim-list-" + i.type),
				l = {};
			if (C[i.type])
				if ("friend" === i.type) layui.each(C.friend, function(e, n) {
					if (i.groupid == n.id) return layui.each(C.friend[e].list, function(e, t) {
						if (t.id == i.id) return a = !0
					}), a ? t.msg("好友 [" + (i.username || "") + "] 已经存在列表中", {
						anim: 6
					}) : (C.friend[e].list = C.friend[e].list || [], l[C.friend[e].list.length] = i, i.groupIndex = e, C.friend[
						e].list.push(i), !0)
				});
				else if ("group" === i.type) {
			...
			if (!a) {
				...
				if ("friend" === i.type) {
					...
					o.find(".layui-layim-list").append(s), o.find(".layim-count").html(C.friend[i.groupIndex].list.length), o.find(
			...
			C[i.type] && ("friend" === i.type ? layui.each(C.friend, function(e, t) {
				layui.each(t.list, function(t, n) {
					if (i.id == n.id) {
						...
						return l.find(".layui-layim-list>li").eq(t).remove(), C.friend[e].list.splice(t, 1), l.find(".layim-count")
							.html(C.friend[e].list.length), 0 === C.friend[e].list.length && l.find(".layui-layim-list").html(
								'<li class="layim-null">该分组下已无好友了</li>'), !0
					}
				})
			}) : "group" === i.type && layui.each(C.group, function(e, t) {
				if (i.id == t.id) return a.find(">li").eq(e).remove(), C.group.splice(e, 1), 0 === C.group.length && a.html(
		...
		$ = function() {
			...
			a.focus(), a.off("keydown").on("keydown", function(i) {
				var e = layui.data("layim")[C.mine.id] || {},
					t = i.keyCode;
				if ("Ctrl+Enter" === e.sendHotKey) return void(i.ctrlKey && 13 === t && O());
				if (13 === t) {
		...
		U = {
			status: function(i, a) {
				var t = function() {
					...
					n = i.attr("lay-type");
				if ("show" === n) B(a), i.next().show().addClass(Y), e(document).off("click", t).on("click", t);
				else {
					var l = i.parent().prev();
					i.addClass(s).siblings().removeClass(s), l.html(i.find("cite").html()), l.removeClass("layim-status-" + (
			...
			tab: function(i) {
				...
				"number" == typeof i ? (a = i, i = t.eq(a)) : a = i.index(), a > 2 ? t.removeClass(s) : (U.tab.index = a, i.addClass(
					s).siblings().removeClass(s)), p.find(e).eq(a).addClass(l).siblings(e).removeClass(l)
			},
			spread: function(i) {
					...
					n = function(i) {
						...
						if ("" === a) U.tab(0 | U.tab.index);
							...
							for (var n = [], l = C.friend || [], s = C.group || [], o = "", r = 0; r < l.length; r++)
								for (var d = 0; d < (l[r].list || []).length; d++) l[r].list[d].username.indexOf(a) !== -1 && (l[r].list[d].type =
									"friend", l[r].list[d].index = r, l[r].list[d].list = d, n.push(l[r].list[d]));
							for (var u = 0; u < s.length; u++) s[u].groupname.indexOf(a) !== -1 && (s[u].type = "group", s[u].index = u,
								s[u].list = u, n.push(s[u]));
							if (n.length > 0)
								for (var c = 0; c < n.length; c++) o += '<li layim-event="chat" data-type="' + n[c].type + '" data-index="' +
									n[c].index + '" data-list="' + n[c].list + '"><img src="' + n[c].avatar + '"><span>' + (n[c].username || n[
										c].groupname || "佚名") + "</span><p>" + (n[c].remark || n[c].sign || "") + "</p></li>";
							else o = '<li class="layim-null">无搜索结果</li>';
							e.html(o), U.tab(3)
						}
					};
				!C.base.isfriend && C.base.isgroup ? U.tab.index = 1 : C.base.isfriend || C.base.isgroup || (U.tab.index = 2), a
			...
			chat: function(i) {
				...
				"friend" === e ? l = C[e][t].list[n] : "group" === e ? l = C[e][n] : "history" === e && (l = (a.history || {})[t] ||
					{}), l.name = l.name || l.username || l.groupname, "history" !== e && (l.type = e), L(l)
			},
			tabChat: function(i) {
				z(i)
			},
			...
			groupMembers: function(i, a) {
				...
				i.data("down") ? l() : (n.html("&#xe619;"), i.data("down", !0), U.groupMembers.index = t.tips(
			...
			setSend: function(i, a) {
				var t = i.siblings(".layim-menu-box"),
					...
					l = i.attr("lay-type");
				if ("show" === l) B(a), t.show().addClass(Y), e(document).off("click", n).on("click", n);
				else {
			...
			image: function(i) {
				...
				layui.upload({
					...
					type: a,
					success: function(i) {
						0 == i.code ? (i.data = i.data || {}, "images" === a ? F(n.textarea[0], "img[" + (i.data.src || "") + "]") :
								"file" === a && F(n.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]")) :
							t.msg(i.msg || "上传失败")
					}
				})
			},
	...
	i("layim", new d)
}).addcss("modules/layim/layim.css?v=2.092", "skinlayimcss");
